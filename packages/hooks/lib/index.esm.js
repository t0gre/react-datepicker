      ...
      (G = r.timezone),
        (I =
          ((N = F.exec(G))
            ? 0
            : (N = Y.exec(G))
            ? ((C = 60 * parseInt(N[2], 10)), '+' === N[1] ? -C : C)
            : (N = S.exec(G))
            ? ((C = 60 * parseInt(N[2], 10) + parseInt(N[3], 10)), '+' === N[1] ? -C : C)
            : 0) * i)
    else {
      var W = O + A,
        L = new Date(W)
...
function Fe(e, t, n) {
  ...
  return Array.from(Array(t).keys()).reduce(function(e) {
    return (
      (r = 0 === e.length ? he(r, n) : he(r, n >= 0 ? 1 : -1)),
      n > 0 ? e.concat([pe(r)]) : [pe(r)].concat(e)
    )
  }, [])
}
function Ye(e, t, n) {
