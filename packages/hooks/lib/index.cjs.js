...
function parse(e, t) {
  ...
  var n = splitDateString(e),
    a = parseYear(n.date, r),
    o = a.year,
    s = parseDate(a.restDateString, o)
  if (s) {
    var i,
      u = s.getTime(),
      d = 0
    if ((n.time && (d = parseTime(n.time)), n.timezone))
      ...
      var f = u + d,
        c = new Date(f)
      i = getTimezoneOffsetInMilliseconds(c)
      var D = new Date(f)
      D.setDate(c.getDate() + 1)
      var _ = getTimezoneOffsetInMilliseconds(D) - getTimezoneOffsetInMilliseconds(c)
      _ > 0 && (i += _)
    }
...
function splitDateString(e) {
  ...
  if (
    (parseTokenPlainTime.test(n[0]) ? ((r.date = null), (t = n[0])) : ((r.date = n[0]), (t = n[1])),
    t)
  ) {
    var a = parseTokenTimezone.exec(t)
...
function parseYear(e, t) {
  var r,
    n = parseTokensYYY[t],
    a = parseTokensYYYYY[t]
  if ((r = parseTokenYYYY.exec(e) || a.exec(e))) {
    var o = r[1]
    return {year: parseInt(o, 10), restDateString: e.slice(o.length)}
  }
  if ((r = parseTokenYY.exec(e) || n.exec(e))) {
    var s = r[1]
...
function parseDate(e, t) {
  ...
  if ((r = parseTokenMMDD.exec(e))) {
    ...
    return n.setUTCFullYear(t, a, s), n
  }
...
function parseTime(e) {
  ...
  if ((t = parseTokenHHMM.exec(e)))
    return (
      ...
      (r % 24) * MILLISECONDS_IN_HOUR + n * MILLISECONDS_IN_MINUTE$1
    )
  if ((t = parseTokenHHMMSS.exec(e))) {
    ;(r = parseInt(t[1], 10)), (n = parseInt(t[2], 10))
    var a = parseFloat(t[3].replace(',', '.'))
    return (r % 24) * MILLISECONDS_IN_HOUR + n * MILLISECONDS_IN_MINUTE$1 + 1e3 * a
  }
  return null
}
...
function differenceInCalendarDays(e, t) {
  var r = start_of_day(e),
    n = start_of_day(t),
    a = r.getTime() - r.getTimezoneOffset() * MILLISECONDS_IN_MINUTE$2,
...
function format(e, t, r) {
  ...
  a &&
    ...
    a.format.formattingTokensRegExp && (s = a.format.formattingTokensRegExp))
  var i = parse_1(e)
...
function buildFormatFn(e, t, r) {
  var n,
    ...
    s = o.length
  for (n = 0; n < s; n++)
    (a = t[o[n]] || formatters[o[n]]), (o[n] = a || removeFormattingTokens(o[n]))
  return function(e) {
    for (var t = '', r = 0; r < s; r++)
...
function formatTimezone(e, t) {
  ...
  var r = e > 0 ? '-' : '+',
    n = Math.abs(e),
    a = n % 60
  return r + addLeadingZeros(Math.floor(n / 60), 2) + t + addLeadingZeros(a, 2)
}
...
function eachDay(e, t, r) {
  ...
  var s = [],
    i = n
  for (i.setHours(0, 0, 0, 0); i.getTime() <= o; ) s.push(parse_1(i)), i.setDate(i.getDate() + a)
  return s
}
...
function getDays(e) {
  var t = e.year,
    ...
    d = get_day(u),
    f = end_of_month(i),
    c = Array.from(Array(d >= a ? d - a : a).keys()).fill(0),
    D = each_day(u, f).map(function(e) {
...
function useMonth(e) {
  ...
  return {
    days: react.useMemo(
      function() {
        return getDays({year: t, month: r, weekStartsOn: a, dayFormat: s})
      },
      [t, r, a, s],
    ),
    weekDays: react.useMemo(
      function() {
        return getWeekDays({weekStartsOn: a, weekDayFormat: u})
      },
      [a, u],
    ),
    monthLabel: f(new Date(t, r)),
  }
}
...
function isDateBlocked(e) {
  var t = e.date,
    ...
    f = n ? new Date(n.getFullYear(), n.getMonth(), n.getDate(), 0, 0, 0) : n
  return !!(
    (d && is_before(t, d)) ||
    (f && is_after(t, f)) ||
...
function canSelectRange(e) {
  ...
  if (t && a > 1 && !r)
    return each_day(t, add_days(t, a - 1)).reduce(function(e, t) {
      return e ? !n(t) : e
    }, !0)
  if (t && r) {
    var o = add_days(t, a - 1)
    return (
      !is_before(r, o) &&
...
function isDateHovered(e) {
  var t = e.date,
    ...
    o = e.hoveredDate
  return (
    !(!r || n || !o || is_before(o, r) || !is_within_range(t, r, o)) &&
...
function useDatepicker(e) {
  var t = e.startDate,
    ...
    k = react.useCallback(
      function(e) {
        return isDateHovered({date: e, hoveredDate: y, startDate: t, endDate: r, isDateBlocked: g})
      },
      [y, t, r, g],
    )
  return {
    ...
    onDayHover: function(e) {
      !t ||
      r ||
      (a && o && !is_within_range(e, a, o)) ||
      (u > 1 && t && is_within_range(e, t, add_days(t, u - 2)))
        ? p(null)
        : p(e)
    },
    onDaySelect: function(e) {
      ;((n === END_DATE && t && is_before(e, t)) || (n === START_DATE && r && is_after(e, r))) &&
      canSelectRange({minBookingDays: u, isDateBlocked: g, startDate: e, endDate: null})
        ? s({endDate: null, startDate: e, focusedInput: END_DATE})
        : n === START_DATE &&
          canSelectRange({minBookingDays: u, isDateBlocked: g, endDate: r, startDate: e})
        ? s({endDate: r, startDate: e, focusedInput: END_DATE})
        : n === START_DATE &&
          canSelectRange({minBookingDays: u, isDateBlocked: g, endDate: null, startDate: e})
        ? s({endDate: null, startDate: e, focusedInput: END_DATE})
        : n === END_DATE &&
          t &&
          !is_before(e, t) &&
          canSelectRange({minBookingDays: u, isDateBlocked: g, startDate: t, endDate: e}) &&
          s({startDate: t, endDate: e, focusedInput: null})
    },
    goToPreviousMonths: function() {
      T(getNextActiveMonth(m, f, -1))
    },
